module.exports = function(req, res) {
  ...
  // http://localhost:3000/topup?userId=1&deno=100
  var params = req.body;
  // console.log(params);
  ...
  var thisUser, lastRefCode;
  Q.ninvoke(User, 'get', params.userId)
  .then(function(xUser) {
    thisUser = xUser;
    
    return Q.ninvoke(CreditsHistory, 'getLatestHistory', thisUser.id);
  })
  .then(function(latestHistory) {

    auth.getAccessToken(thisUser.code, function(req, res) {
      var data = res.body;
      ...
      if (res.statusCode === 200 && data && data['access_token']) {
        ...
        CreditsHistory.getLastRefCode(function(err, data) {
          var lastRefCode = parseInt(data) + 1;
          if (err) {
            throw new Error('cannot get last ref code');
          }
